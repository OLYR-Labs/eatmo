...
export default function AdminDashboard({ userName }: { userName: string }) {
  ...
  async function addCategory(e: FormEvent) {
    e.preventDefault(); if (!newCategory.trim()) return; setSaving("new"); setError("");
    try { await api("POST", { type: "category", name: newCategory.trim() }); setNewCategory(""); await load(); } catch (e) { setError(e instanceof Error ? e.message : "Unable to add category"); } finally { setSaving(null); }
  }
  async function addItem(categoryId: string) {
    const name = window.prompt("New menu item name"); if (!name?.trim()) return; setSaving(`add-${categoryId}`); setError("");
    try { await api("POST", { type: "item", categoryId, name: name.trim() }); await load(); } catch (e) { setError(e instanceof Error ? e.message : "Unable to add item"); } finally { setSaving(null); }
  }
  async function save(type: "category" | "item", value: Category | Item) {
    setSaving(value.id); setError("");
    try { await api("PATCH", { type, ...value }); await load(); } catch (e) { setError(e instanceof Error ? e.message : "Unable to save"); } finally { setSaving(null); }
  }
  async function remove(type: "category" | "item", id: string) {
    ...
    try { await api("DELETE", { type, id }); await load(); } catch (e) { setError(e instanceof Error ? e.message : "Unable to delete"); } finally { setSaving(null); }
  }
  ...
  <form onSubmit={addCategory} className="flex flex-col gap-3 rounded-3xl bg-white p-5 shadow-sm sm:flex-row"><input value={newCategory} onChange={e => setNewCategory(e.target.value)} placeholder="New category name" className="min-w-0 flex-1 rounded-xl border border-gray-200 px-4 py-3 outline-none focus:border-purple-500" /><button disabled={saving === "new"} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#5f31b4] px-5 py-3 font-bold text-white disabled:opacity-50"><Plus size={18} />Add category</button></form>
  <div className="space-y-6">{categories.map(category => <section key={category.id} className="overflow-hidden rounded-3xl bg-white shadow-sm"><div className="border-b border-gray-100 bg-gray-50 p-5"><div className="grid gap-3 md:grid-cols-[1fr_auto_auto] md:items-end"><label className="text-xs font-bold uppercase tracking-wider text-gray-500">Category name<input value={category.name} onChange={e => updateCategory(category.id, { name: e.target.value })} className="mt-2 w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-lg font-bold outline-none focus:border-purple-500" /></label><label className="flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm font-semibold"><input type="checkbox" checked={category.active} onChange={e => updateCategory(category.id, { active: e.target.checked })} />Visible</label><div className="flex gap-2"><button onClick={() => save("category", category)} disabled={saving === category.id} className="inline-flex items-center gap-2 rounded-xl bg-[#5f31b4] px-4 py-3 text-sm font-bold text-white disabled:opacity-50"><Save size={16} />Save</button><button onClick={() => remove("category", category.id)} className="rounded-xl border border-red-200 p-3 text-red-600"><Trash2 size={16} /></button></div></div></div><div className="divide-y divide-gray-100">{category.items.map(item => <div key={item.id} className="p-5"><div className="grid gap-3 md:grid-cols-12"><input value={item.name} onChange={e => updateItem(category.id, item.id, { name: e.target.value })} className="rounded-xl border border-gray-200 px-3 py-2.5 font-semibold md:col-span-3" placeholder="Item name" /><input value={item.description ?? ""} onChange={e => updateItem(category.id, item.id, { description: e.target.value })} className="rounded-xl border border-gray-200 px-3 py-2.5 md:col-span-3" placeholder="Description" /><input value={item.price ?? ""} onChange={e => updateItem(category.id, item.id, { price: num(e.target.value) })} type="number" className="rounded-xl border border-gray-200 px-3 py-2.5" placeholder="Price" /><input value={item.small ?? ""} onChange={e => updateItem(category.id, item.id, { small: num(e.target.value) })} type="number" className="rounded-xl border border-gray-200 px-3 py-2.5" placeholder="Small" /><input value={item.large ?? ""} onChange={e => updateItem(category.id, item.id, { large: num(e.target.value) })} type="number" className="rounded-xl border border-gray-200 px-3 py-2.5" placeholder="Large" /><input value={item.unit ?? ""} onChange={e => updateItem(category.id, item.id, { unit: e.target.value })} className="rounded-xl border border-gray-200 px-3 py-2.5" placeholder="Unit" /><label className="flex items-center justify-center gap-2 rounded-xl border border-gray-200 px-3 py-2.5 text-sm font-semibold"><input type="checkbox" checked={item.available} onChange={e => updateItem(category.id, item.id, { available: e.target.checked })} />Available</label><div className="flex gap-2 md:col-span-2"><button onClick={() => save("item", item)} disabled={saving === item.id} className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#5f31b4] px-3 py-2.5 text-sm font-bold text-white disabled:opacity-50"><Save size={16} />Save</button><button onClick={() => remove("item", item.id)} className="rounded-xl border border-red-200 p-2.5 text-red-600"><Trash2 size={16} /></button></div></div></div>)}</div><div className="border-t border-gray-100 p-5"><button onClick={() => addItem(category.id)} disabled={saving === `add-${category.id}`} className="inline-flex items-center gap-2 rounded-xl border border-purple-200 bg-purple-50 px-4 py-2.5 text-sm font-bold text-purple-700"><Plus size={16} />Add menu item</button></div></section>)}</div></div></main>;
}
