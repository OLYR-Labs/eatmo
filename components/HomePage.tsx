...
const MENU_QUERY = `*[_type == "menuCategory"] | order(order asc) { title, order, items[] { name, price, small, large, unit }, notes }`;
...
async function getMenu(): Promise<MenuCategory[]> {
  ...
  try {
    ...
    const categories = await client.fetch<SanityMenuCategory[]>(MENU_QUERY, {}, { cache: "no-store" });
      ...
      <div className="container grid gap-8 lg:grid-cols-[.75fr_1.25fr]">
        ...
        <div className="soft bg-white p-7 shadow-sm md:p-10">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            <Feature icon="🍽️" title="Freshly prepared" text="Food made for the table." />
            <Feature icon="🌶️" title="Full of flavour" text="Sri Lankan favourites." />
            <Feature icon="👨‍👩‍👧‍👦" title="Made to share" text="Perfect for groups." />
          </div>
        </div>
      </div>
    </section>
...
function Feature({ icon, title, text }: { icon: string; title: string; text: string }) {
  return <div className="rounded-3xl bg-[var(--purple-soft)] p-5"><div className="text-2xl">{icon}</div><h3 className="mt-3 font-bold">{title}</h3><p className="mt-1 text-sm text-gray-600">{text}</p></div>;
}
      ...
      <div className="container grid gap-10 lg:grid-cols-[.85fr_1.15fr]">
        <div>
          ...
          <div className="mt-8 space-y-5 text-gray-300">
            <ContactRow icon={<MapPin />} title="Location"><p>P5VH+7MH, A8, Ingiriya, Sri Lanka</p></ContactRow>
            <ContactRow icon={<Phone />} title="Call"><a href={`tel:+${PHONE}`} className="transition hover:text-white">{DISPLAY_PHONE}</a></ContactRow>
            <ContactRow icon={<Clock3 />} title="Opening hours"><p>Daily · 10:00 AM–12:00 AM</p></ContactRow>
          </div>
          <div className="mt-8 flex flex-wrap gap-3">
...
function ContactRow({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) {
  return <div className="flex gap-4"><span className="mt-1 shrink-0 text-purple-300">{icon}</span><div><p className="font-bold text-white">{title}</p>{children}</div></div>;
}
